...
<div class="container">
    <div class="card w-50 mx-auto">
        <div class="m-3">
            <form class="form-group" action="{{route('giftcards.update', $gc->id)}}" method="POST">
            @method('PATCH')
            @csrf
            <div class="form-group">
                <h5 class="text-uppercase">Create GiftCards</h5>
                <label>GiftCard Points Label</label>
                <input type="text" class="form-control" name="gc_name" value="{{$gc->gc_name}}">
            </div>
                <label>Price</label>
                <input type="text" class="form-control" name="points" value="{{$gc->points}}">
